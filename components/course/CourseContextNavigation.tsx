...
export function CourseContextNavigation({ courseSlug, capabilities, lessonSlug, activeMode }: { courseSlug: string; capabilities: CourseCapabilities; lessonSlug?: string; activeMode: "view" | "learn" | "edit" | "publication" }) {
  ...
  return <div className="course-context-navigation"><Link className="course-back-link" href="/app/courses"><ArrowLeft size={15} /> Mes parcours</Link><nav className="mode-switch" aria-label="Contexte du parcours">{getCourseContextLinks(courseSlug, capabilities, lessonSlug).map((link) => { const Icon = icons[link.label]; return <Link aria-current={link.label === activeLabel ? "page" : undefined} href={link.href} key={link.label}><Icon size={15} />{link.label}</Link>; })}</nav></div>;
}
