...
export function CourseOutlineRail({ courseSlug, outline, selectedLesson, mode }: { courseSlug: string; outline: CourseOutline; selectedLesson?: string; mode?: "learn" | "edit" | "preview" }) {
  const suffix = mode === "edit" ? "?mode=edit" : mode === "preview" ? "?mode=preview" : "";
  const lessons = outline.flatMap((m) => m.lessons);
  const completed = lessons.filter((l) => l.status === "completed").length;
  return <WorkspaceRail panel="structure">
    {mode === "learn" && lessons.length > 0 && <div className="outline-progress"><span>Votre progression</span><strong>{completed} / {lessons.length} leçons</strong><progress aria-label="Progression du parcours" max={lessons.length} value={completed} /></div>}
