...
function LessonEditorDraft({ courseId, courseSlug, lesson }: { courseId: string; courseSlug: string; lesson: CourseLesson }) {
  ...
  const tabs = ["info", "content", "resources"] as const;
  ...
  return <><form className="form authoring-form" action={save}>
    <p className="caption">Leçon sélectionnée</p><h2>{draft.title}</h2><div className="editor-tabs" role="tablist" aria-label="Édition de la leçon">{tabs.map((item, index) => <button id={`lesson-tab-${item}`} key={item} type="button" role="tab" aria-selected={tab === item} tabIndex={tab === item ? 0 : -1} onKeyDown={(event) => moveTab(index, event.key)} onClick={() => setTab(item)}>{labels[item]}</button>)}</div>
