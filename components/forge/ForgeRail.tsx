...
export function ForgeRail({ context, availability }: { context: ForgeRailContext; availability: ForgeAvailability }) {
  ...
  useEffect(() => {
    let active = true;
    listForgeSourcesAction({ courseSlug: context.courseSlug, mode: context.mode }).then((options) => { if (active) setSources(options); });
    return () => { active = false; };
