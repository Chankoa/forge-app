...
test("public provider keeps a useful business suggestion without requiring a stored domain", async () => {
  const result = await runPublicCoursePreview({ intent: "Manager une équipe dans un centre de vacances" }, { provider: { availability: "configured", async generatePublic(messages) { assert.match(messages.system, /vrai nom de domaine métier/); return { output: { ...preview, suggestedDomainLabel: "Management d'équipe" }, finishReason: "stop" }; } }, consumeRateLimit() {} });
  assert.equal(result.ok && result.preview.suggestedDomainLabel, "Management d'équipe");
  const placeholder = await runPublicCoursePreview({ intent: "Manager une équipe dans un centre de vacances" }, { provider: { availability: "configured", async generatePublic() { return { output: { ...preview, suggestedDomainLabel: "Domaine suggéré" }, finishReason: "stop" }; } }, consumeRateLimit() {} });
  assert.equal(placeholder.ok && placeholder.preview.suggestedDomainLabel, null);
});
