...
export const lessonSchema = z.object({ title: z.string().trim().min(2).max(220), description: z.string().trim().max(1000).optional(), content: z.string().max(20000).optional(), objectives: z.array(z.string().trim().min(1).max(300)).max(8).optional() });
