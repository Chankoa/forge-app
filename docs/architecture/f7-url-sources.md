...
Unit tests cover URL protocol/host/IP/credential rejection, Node's `lookup({ all: true })` callback form, extraction, empty content, ready/failed source resolution, and hostile source placement. Existing Forge tests cover owner capabilities, TXT/MD paths, bounded context and provenance.

F7.1 validated local outbound network with `curl.exe` against `example.com` and IANA. A real owner action persisted `https://example.com`, survived refresh, and reappeared as a ready URL source. A real provider response used the `Example Domain` witness and reported that exact source in `sourcesUsed`. URL deselection removed attribution; selected TXT/MD plus URL reported both truthful source titles. UI tests at 1440, 900, 430 and 390 had no horizontal overflow; Light/Dark, keyboard focus and clean console were verified. Localhost, private IPv4, metadata IPv4 and a public 404 URL were rejected without adding a source. The live schema/RLS audit remains historical F4 evidence, not a fresh F7 catalog audit.
